...
import agent_local.logging_config as logging_config
from agent_local.logging_config import configure_logging
...
def test_configure_logging_registra_stream_e_rotating_file_handler(tmp_path: Path, monkeypatch) -> None:
    ...
    configure_logging("agent-local", "INFO")
    root = logging.getLogger()
...
def test_configure_logging_grava_mensagem_real_no_arquivo(tmp_path: Path, monkeypatch) -> None:
    ...
    configure_logging("agent-local", "INFO")
    marker = "teste-issue-79-agent-local"
    ...
    assert last["service_name"] == "agent-local"
