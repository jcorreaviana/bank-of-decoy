...
- **v43**: correção de robustez no `agent-local`, independente de issue formal — `is_issue_open` (`github_client.py`) propagava `CalledProcessError` quando a seção "Dependências" de uma issue cita um número que não existe mais no GitHub (achado real: a issue #50 do backlog cita "issue #11"/"issue #9" como contexto histórico já concluído, números que não correspondem a nenhuma issue real). Como essa função é chamada por `pick_candidate_issue()`, fora do try/except de `process_issue()` (specs/tech/error-handling.md), a exceção derrubaria o loop inteiro do daemon no primeiro ciclo em que encontrasse essa issue — um gap anterior ao alcance do mecanismo de retry/escalonamento das issues #40/#41, que só age depois de uma issue já ter sido selecionada e atribuída. Corrigido para logar aviso e tratar como "não bloqueia" em vez de propagar. Encontrado durante a implementação da issue #54 (o script reusa a mesma lógica de `pick_candidate_issue` para detectar atividade pendente do agent-local), mas registrado em entrada própria por não ser específico da Fase 2b — vale por si só como correção de robustez do agente.
- **v44**: issue #54 fechada — `scripts/validation_window.py` implementado (Fase 2b, specs/business/24-camada-caos-avancada.md, seção "Execução orgânica"): runbook operacional que sobe o ambiente efêmero (`docker-compose.test.yml`, não o principal — decisão confirmada com o usuário, consistente com v12/v16 de não misturar tráfego sintético/caos com o dataset persistente de ML), aplica `alembic upgrade head` nos 5 bancos de forma idempotente (achado real: sem esse passo automatizado, o ambiente efêmero recém-criado só tem os bancos vazios — toda chamada de API falha silenciosamente com 500, gap já registrado em `docs/licoes-aprendidas-operacao-real.md` para a primeira janela) e roda o gerador de tráfego sintético + `chaos-orchestrator` simultaneamente, repetindo o cenário de cascata de exemplo da issue #53 em ciclos ao longo da janela — decisão de repetir em vez de uma ativação isolada, para gerar volume de sinal suficiente para os agentes ao longo de 2h. Ao fim do relógio (ou `Ctrl+C`), para novas ações mas nunca derruba o ambiente nem interrompe os agentes (daemons externos, fora do controle deste script) — a janela só é considerada finalizada quando não há mais issue candidata nova sem assignee nem issue atribuída ao agent-local sem PR/decisão, reusando o filtro real de `pick_candidate_issue` (mesmo venv do agent-local, para não arriscar drift) em vez de duplicar essa lógica; `agent-stuck` (issues #40/#41) conta como estado terminal, sem teto artificial de tempo para a espera. Validado ao vivo contra o ambiente real: duas execuções curtas (10 min e 3 min) confirmaram o corte limpo no prazo, a fase de espera e o ambiente permanecendo no ar; a detecção de "issue atribuída = pendência" foi validada em separado com uma issue de teste real criada/atribuída/fechada. Achado colateral corrigido no processo, registrado em entrada própria (v43): `is_issue_open` derrubava o daemon do agent-local ao encontrar um número de dependência inexistente no GitHub.
- **v45**: issue #57 fechada — achado real durante a verificação de critério de aceite da #50 (guarda-chuva da Fase 2b): `agent-preditivo.chaos_status.is_chaos_enabled()` decidia `chaos_ativo` lendo `CHAOS_ENABLED` via `docker inspect` (mecanismo estático da Fase 2 original, issues #11/#21) — nunca o override em runtime introduzido pela issue #51 (`POST /internal/chaos/config`, puramente em memória, nunca toca a variável de ambiente). Como o `chaos-orchestrator` (#53) e a janela de 2h (#54) ativam caos exclusivamente por esse mecanismo, durante toda a janela real `is_chaos_enabled()` retornaria `False` mesmo com caos genuinamente ativo — bugs reais causados por caos ficariam sem a label `chaos-test`, e o agent-local poderia tentar "corrigi-los" como se fossem reais, uma regressão silenciosa em relação à garantia já validada em v29. Corrigido com um novo `GET /internal/chaos/status` (par de leitura do `POST` existente, mesma proteção de header/secret, adicionado a `_EXEMPT_PATHS` do middleware pelo mesmo motivo do `POST`), lendo de uma fonte única extraída para `runtime_config.get_effective_config()` (reusada também por `chaos.middleware._load_config()`, agora um wrapper fino, eliminando qualquer risco de divergência entre o que é reportado e o que é de fato injetado). `is_chaos_enabled()` passa a aceitar `base_url` opcional — consulta o endpoint novo quando presente, caindo para o `docker inspect` antigo se o token não estiver configurado ou a chamada falhar (retrocompatibilidade deliberada). Validado contra o ambiente real (imagens reconstruídas): caos ativado só via `POST` (env var confirmada `false` o tempo todo via `docker inspect`) — `GET /internal/chaos/status` refletiu `enabled: true`, e a função real `is_chaos_enabled('account-service', base_url=...)` retornou `True`; sem `base_url` (mecanismo antigo isolado), retornou `False`, reproduzindo o bug original antes da correção.
- **v46**: issue #50 fechada (guarda-chuva Fase 2b — camada de caos avançada). 7 dos 8 itens do critério de aceite confirmados com evidência real: endpoint `POST /internal/chaos/config` (#51), 4 novos tipos de falha (#52), `chaos-orchestrator` executando cenário de cascata (#53), `degradacao_progressiva` confirmada visualmente como tendência no Grafana, issues de caos seguindo rotuladas `chaos-test` e ignoradas pelo agent-local mesmo após a correção da #57 (validado de ponta a ponta, sem mock, via issue real #58), runbook da janela de 2h (#54) e testes de caminho feliz/erro. Item pendente, aceito como débito técnico e não bloqueante para o fechamento: validação de `kafka_lag`/`kafka_delay` contra um broker Kafka real (hoje só testados com mock), já registrado no comentário de auditoria da #52, sem novidade desde então.
- **v47**: achado operacional durante a primeira janela de validação real de 2h rodada de fato (issue #54), independente de issue formal - confirmação ao vivo do mecanismo de auto-expiração já documentado na v42, mais uma limitação do ambiente de execução usado para operar o runbook. Ao interromper `scripts/validation_window.py` no meio de um ciclo de caos ativo, o mecanismo de stop usado (`TaskStop` do harness do Claude Code, neste ambiente) matou o processo de forma abrupta (equivalente a `TerminateProcess`/`SIGKILL`) em vez de entregar um `Ctrl+C`/`SIGINT` real - nem o `finally` do `chaos-orchestrator` (desligamento explícito) nem o handler de `KeyboardInterrupt` do próprio `validation_window.py` chegaram a rodar, deixando `account-service` (`degradacao_progressiva`) e `onboarding-service` (`kafka_delay`) com caos ativo. Nenhuma mudança de código é possível nem necessária aqui - uma finalização desse tipo (equivalente a `kill -9`) não pode ser interceptada por nenhum handler em nenhuma linguagem, é `TerminateProcess` no nível do SO. A rede de segurança da v42 (`duration_seconds`/`expires_at`, `shared/chaos/chaos/runtime_config.py`) é o mecanismo correto para exatamente esse caso, e funcionou como projetado: confirmado com números reais desta ativação - `account-service` ativado às 01:17:08.761Z com `duration_seconds=360` (expira ~01:23:08.761Z), `onboarding-service` às 01:18:08.721Z com `duration_seconds=300` (expira ~01:23:08.721Z); o desligamento manual via `POST /internal/chaos/config {"enabled": false}` (feito para restaurar o ambiente imediatamente, sem esperar) rodou às 01:23:13Z, poucos segundos depois da expiração natural já prevista. Pior caso possível para o cenário de exemplo (`account_and_queue_cascade.yaml`): até ~6 minutos de exposição residual (duração do passo mais longo da timeline + margem de segurança de 60s), se o kill acontecer bem no instante em que o passo acaba de ativar - aceitável para uso local/validação, não motivou reduzir o teto padrão. Nota operacional para o operador: ao rodar este runbook como tarefa em background sob uma ferramenta que só oferece kill forçado (sem `Ctrl+C` real), o ambiente não fica com caos ativo indefinidamente por causa da rede de segurança acima, mas vale conferir/desligar manualmente via `GET`/`POST /internal/chaos/config` se precisar do ambiente limpo antes do teto natural expirar.
- **v48**: issue #66 fechada — vazamento de isolamento real entre o subprocess do Claude Agent SDK (`agent_local.sdk_invocation.invoke_sdk`) e a working tree do operador, descoberto ao processar a #55 durante a janela de validação da #54. Causa raiz: `cwd` chega corretamente ao subprocess (`ClaudeAgentOptions(cwd=cwd)`, e o transporte do SDK de fato o repassa ao spawn do processo), mas esse mesmo transporte só filtra `CLAUDECODE` do ambiente herdado antes de repassar tudo mais — variáveis que identificam uma sessão IDE anexada (`CLAUDE_CODE_MESSAGING_SOCKET`/`TOKEN`/`SESSION_ID`) passam direto, e o CLI empacotado (`claude_agent_sdk/_bundled/claude.exe`, confirmado via string presente no binário) usa esse canal para se conectar à sessão IDE ativa, resolvendo Read/Edit contra a workspace real aberta no VS Code em vez do clone isolado (`agent-local/workspace/bank-of-decoy`). Gatilho: daemon do `agent-local` lançado como subtarefa em background de uma sessão interativa do Claude Code no VS Code (para orquestrar a #54) — o processo herda o socket/token de mensageria IDE de quem o lançou. Reproduzido de forma determinística e repetível com arquivo-marcador de conteúdo distinto em dois diretórios (working tree real vs. clone isolado), antes e depois da correção. Corrigido com lista positiva (não negativa) de variáveis repassadas ao subprocess (`_minimal_subprocess_env`, `_ALLOWED_ENV_PASSTHROUGH`) — decisão deliberada: `ClaudeAgentOptions(env=...)` só sobrepõe chaves específicas do ambiente herdado (`{**inherited_env, **options.env}`), nunca o substitui por completo, então uma blacklist de variáveis conhecidas por vazar reabriria o mesmo buraco no dia em que uma variável nova aparecesse — a única forma real de garantir uma lista positiva é reduzir o `os.environ` do próprio processo chamador antes da chamada ao SDK, escopo estreito (só ao redor de `invoke_sdk`, não o processo inteiro do daemon, já que `github_client`/`git_ops` não fazem parte da superfície deste vazamento). Escopo do dano investigado: nenhuma das 7 issues já fechadas nesta sessão de trabalho (#40, #41, #44, #45, #51, #52, #53) passou pelo caminho autônomo do `agent-local` — confirmado via busca pela assinatura `via agent-local (Claude Agent SDK)` (`gate.open_pull_request`) em todos os PRs mergeados da história do projeto: só 3 ocorrências (#39/#36, #25/#24, #23/#22), todas com diff de arquivo único no escopo esperado, sem contaminação. Teste de regressão permanente com chamada real ao SDK adicionado (`agent-local/tests/integration/test_isolation_leak.py`) — deliberadamente fora da suíte rápida de `tests/unit/`, já que a falha é silenciosa (sem exceção, sem log de erro) e só é detectável observando o comportamento real do subprocess, não mockando `invoke_sdk`/`query`; validado dentro desta mesma sessão IDE vulnerável, confirmando isolamento após a correção. README documenta recomendação de defesa em profundidade: não lançar o daemon do `agent-local` como subtarefa de uma sessão interativa do Claude Code/VS Code em produção.
- **v49**: registro retroativo — issue #65 fechada (commit `3aab6e6`, cronologicamente anterior à v47/v48, mas nunca registrada aqui; lacuna identificada em auditoria de fechamento posterior). Critério de aceite era o texto de "Correção esperada" da própria issue (não havia checklist formal): (1) destino 2 (`no_action_needed`) deve fechar a issue via `github_client.close_issue`, mesma semântica terminal do caminho com diff (`Closes #N` no PR), em vez de só desatribuir; (2) a branch de trabalho local daquele ciclo deve ser removida (`git_ops.delete_local_branch`), já que nunca chega a ser empurrada nesse caminho. Ambos os itens confirmados implementados em `agent_local/polling.py::_handle_no_action_needed`, com testes de regressão novos (`agent-local/tests/unit/test_git_ops.py`) cobrindo especificamente o cenário de reprodução (reseleção da mesma issue após no-op não colide mais com a branch) e validação manual contra issue real (#64, removida após a validação) registrada na mensagem do commit. Achado colateral da mesma auditoria: as três issues citadas no corpo original da #65 como afetadas pelo bug mecânico (#60, #61, #62) só tiveram o `agent-stuck`/assignee/branch órfã limpos agora, retroativamente, junto com #55 e #59 (residual de sessão de validação anterior, label `backlog-anterior`) — não no momento do merge da correção.
- **v50**: issue #69 fechada — novo `latencia_alta` no `onboarding-service`, causa raiz distinta das anteriores (#34, cifra Fernet recriada por chamada; #37/#38, `httpx.Client` recriado por chamada). Aqui o custo extra estava em `app/services/onboarding_events.py::publish_onboarding_classified`: para os fluxos de reprovação (qualidade/fraude), o mesmo fato de negócio é publicado em dois tópicos Kafka (tópico de resultado + fila de revisão), e cada publicação passava por `publish_event`, que faz `producer.produce` seguido de `producer.flush` bloqueante — dois flushes sequenciais no mesmo request, cada um esperando a confirmação de entrega do broker antes do outro começar, dobrando o tempo de round-trip nesse caminho (o caminho de aprovação, só um tópico, não é afetado — consistente com o sinal ser em p95, não na mediana). Corrigido em `app/core/kafka.py`: `publish_event` (compatibilidade, um evento) agora delega para `publish_events` (lista de eventos), que produz todas as mensagens primeiro e só então chama `producer.flush` uma única vez, deixando o broker confirmar as entregas em paralelo em vez de em série. Testes unitários atualizados (`tests/unit/test_onboarding_events.py`, `tests/unit/test_kafka.py`) para o novo formato de chamada (lista de `(topico, envelope, key)`) e cobrindo o caso de flush único para múltiplos eventos.
- **v51**: issue #80 fechada — `risk_decisions` não registrava custo/tempo de execução do SDK. Colunas `total_cost_usd` (numeric) e `sdk_duration_ms` (bigint) adicionadas via migration `e1a2b3c4d5f6`, nullable (decisões do agent-preditivo não têm invocação de SDK associada). `SDKInvocationResult` passa a capturar `duration_ms` do `ResultMessage`, antes descartado. Validação real contra o Postgres do docker-compose: migration aplicada no schema de fato, e um write real via `agent_local.agent_ops_db.record_risk_decision` confirmado com `SELECT`. Auditoria de fechamento encontrou lacuna real: `gate.py` e `polling.py` já repassavam os campos do `SDKInvocationResult` para `record_risk_decision`, mas nenhum teste afirmava isso — remover essas duas linhas não quebraria a suíte. Testes de regressão adicionados em `test_gate.py` e `test_polling.py` antes do fechamento formal.
- **v52**: correção implementada para a issue #74 (ainda aberta — fechamento manual via `/fecha-issue` pendente), achada durante a auditoria da janela de validação Fase 2b (`docs/relatorio-janela-fase2b.md`, seção 11.2): `agent_local/sdk_invocation.py::_deny_out_of_scope_tools` permitia `Edit` incondicionalmente, sem checar se o caminho editado caía dentro do `cwd` isolado da invocação — mesma classe de falha da issue #66 (vazamento de isolamento entre o subprocess do SDK e a working tree real do operador), mas por um canal diferente: a #66 era vazamento de variável de ambiente de sessão IDE herdada pelo subprocess; aqui não há sessão IDE nem variável vazada nenhuma — é o próprio modelo, dentro de uma execução isolada e limpa, decidindo editar um caminho absoluto (ou relativo via `../`) fora do `cwd` pretendido, sem nenhuma barreira técnica impedindo. Evidência real: 3 de 10 issues da janela (#59, #69, #70) tiveram edições reais vazando para a working tree do operador por esse canal, fazendo `diff_lines` (calculado contra o clone isolado) não refletir o fix real e essas issues serem classificadas incorretamente como `no_action_needed`.

  Implementação em duas partes, a segunda descoberta só ao testar contra o SDK real (não pela leitura do código): (1) `_deny_out_of_scope_tools` virou uma fábrica (`_make_deny_out_of_scope_tools(cwd)`, closure vinculada ao `cwd` da invocação — `ToolPermissionContext` do SDK não carrega `cwd`) que resolve o caminho absoluto de qualquer `Edit` e nega (não ignora silenciosamente) qualquer caminho fora do prefixo do `cwd`, mesmo princípio de whitelist já usado em `_ALLOWED_ENV_PASSTHROUGH` (#66), não blacklist; (2) achado real maior que o esperado, só descoberto rodando `tests/integration/test_isolation_leak.py` contra o SDK de verdade: `ALLOWED_TOOLS` tinha `"Edit"` como entrada inteira (sem especificador), o que blinda (`CanUseToolShadowedWarning` do próprio SDK) `can_use_tool` para toda chamada de `Edit` — o callback do item (1) nunca era sequer consultado em uma execução real, mesmo depois de implementado. Testado com um espião em volta do callback contra o SDK real (3 rodadas): com `ALLOWED_TOOLS` sem nenhuma entrada de `Edit`, toda edição (mesmo legítima, dentro do `cwd`) foi negada com mensagem genérica de `"don't ask mode"` do próprio CLI, e o callback nunca foi chamado; com `"Edit"` inteiro (config antiga), toda edição foi aprovada, e o callback nunca foi chamado; com a entrada escopada `"Edit(**)"` (especificador real, não `""`/`"*"`), o motor de regras do próprio CLI passou a resolver `**` relativo ao `cwd` da invocação e a negar/permitir corretamente sozinho — ainda sem consultar o callback. Ou seja: sob `permission_mode="dontAsk"`, quem bloqueia `Edit` de verdade hoje é a entrada `"Edit(**)"` em `ALLOWED_TOOLS`, não `can_use_tool` — o callback do item (1) foi mantido mesmo assim como defesa em profundidade barata e documentação explícita do escopo pedido pela issue (e volta a ser a proteção real se uma atualização futura do CLI parar de blindar entradas escopadas desse jeito).

  Validado contra o SDK real (não só mock), rodando `agent-local/tests/integration/test_isolation_leak.py` de ponta a ponta 3 vezes com prompts reais: edição legítima dentro do `cwd` isolado (teste de regressão original da #66, que passou a falhar com a correção incompleta do item (2) sozinho e voltou a passar com `"Edit(**)"`), tentativa de edição em caminho absoluto fora do `cwd` (negada, arquivo externo não alterado) e tentativa de edição via caminho relativo `../` escapando do `cwd` (negada, mesmo sem o teste automatizado permanente — validação manual). Suíte unitária (`agent-local/tests/unit/test_sdk_invocation.py`) ganhou cobertura da checagem de caminho isolada (`_make_deny_out_of_scope_tools`, caminho absoluto fora do `cwd`, `../` relativo, `file_path` ausente, caminho dentro do `cwd`) e da composição real de `ALLOWED_TOOLS`. Não validado nesta correção: um ciclo completo do daemon `agent-local` processando uma issue real do GitHub de ponta a ponta (criação de issue/branch/PR reais) — os testes de integração contra o SDK real, que exercitam exatamente o mecanismo corrigido (`invoke_sdk`/`ALLOWED_TOOLS`/`can_use_tool`) sem side effects no GitHub, foram considerados evidência suficiente para esta correção pontual; fica como validação adicional natural na próxima janela de validação real (#54-like).
- **v53**: fecha a lacuna deixada em aberto pela v52 — ciclo real do daemon `agent-local` (não script avulso: `python -m agent_local.polling --once`, o mesmo entrypoint de produção) exercitado de ponta a ponta contra a correção da #74, antes do fechamento formal da issue. Issue de teste temporária criada (#82, label `bug`, mudança trivial e inequívoca: duas linhas em `README.md`), com a #74 temporariamente auto-atribuída durante a janela de teste para não competir por seleção (`list_candidate_issues` exige `no:assignee`) — restaurada a não atribuída ao final. Achado colateral do próprio processo, não relacionado à correção da #74: o clone isolado (`agent-local/workspace/bank-of-decoy`) tinha mudanças não commitadas de uma sessão anterior (arquivos do fix da #69, `onboarding-service/app/core/kafka.py` e afins) bloqueando `git pull --ff-only` — confirmado idêntico ao conteúdo já mergeado em `origin/main` (`git diff origin/main` vazio) antes de descartar, sem perda de trabalho real. Resultado do ciclo real: candidata #82 selecionada corretamente, SDK editou só `README.md` dentro do `cwd` isolado (`diff_lines=2`, refletindo corretamente a edição — o próprio problema que a #74 corrigia), PR #83 aberto com o diff exato esperado, decisão `humano` (score 78.10 vs threshold 20 do tier crítico, criticidade marcada deliberadamente como crítico na issue de teste para não arriscar merge automático em `main`). Nenhuma tentativa de escrita fora do `cwd` ocorreu durante o ciclo (nada para o callback negar desta vez - o cenário testado foi o caminho feliz, dentro do escopo; o caminho de negação já tem cobertura real via `test_isolation_leak.py`, v52). Confirmado sem vazamento para a working tree real: `git status` limpo e o marcador de teste ausente do `README.md` real durante e depois do ciclo. Limpeza sem resíduo: PR #83 fechado sem merge, branch remota e local `agent-local/issue-82` removidas, issue #82 fechada com o resumo da validação, #74 restaurada ao estado original (aberta, sem assignee).
- **v54**: correção implementada para a issue #75 (fechamento manual via `/fecha-issue` pendente), causa raiz confirmada no relatório da janela real de 2h (`docs/relatorio-janela-fase2b.md`, seção "Aviso crítico que muda a leitura de várias seções abaixo: o caos nunca foi injetado"): os 40 ciclos do `chaos-orchestrator` na janela falharam de forma idêntica com `FileNotFoundError`, sem nenhum caos injetado em nenhum momento. Causa raiz: `scripts/validation_window.py::run_chaos_cycle` invoca `orchestrator.py` com `cwd=CHAOS_ORCHESTRATOR_DIR` (o subprocesso já roda de dentro de `chaos-orchestrator/`), mas o `--scenario` passado explicitamente naquela execução usava um caminho relativo à raiz do repo (`chaos-orchestrator\scenarios\account_and_queue_cascade.yaml`) — resolvido pelo subprocesso como `chaos-orchestrator/chaos-orchestrator/scenarios/...`, inexistente. O `DEFAULT_SCENARIO` do próprio script (`Path` absoluto) nunca teve esse problema; a falha só ocorre quando um argumento explícito relativo sobrepõe o default, exatamente o que aconteceu na janela real.

  Corrigido com `resolve_scenario_path()` novo em `validation_window.py`: um caminho absoluto passa direto (comportamento do `DEFAULT_SCENARIO` preservado); um caminho relativo é resolvido contra `REPO_ROOT` (não contra o cwd do processo atual nem do subprocesso), então `--scenario chaos-orchestrator/scenarios/x.yaml` funciona do jeito mais natural de digitar o argumento (copiado da estrutura do próprio repo), sem o usuário precisar saber que o subprocesso roda de dentro de `chaos-orchestrator/`. Chamado uma única vez em `main()`, logo após a checagem de `CHAOS_INTERNAL_TOKEN` e antes de subir o ambiente/aplicar migrations — falha rápido com `SystemExit` apontando o caminho absoluto resolvido que foi tentado, em vez de deixar rodar até 40 ciclos (2h) para só então descobrir o problema no relatório final. `run_chaos_cycle`/`chaos_cycle_loop` continuam recebendo o `Path` já resolvido, sem mudança de assinatura.

  Testes novos em `scripts/tests/test_validation_window.py` (`resolve_scenario_path`, com `REPO_ROOT` isolado via `monkeypatch` + `tmp_path`, sem tocar o repo real): caminho relativo resolve corretamente contra a raiz do repo; caminho absoluto passa direto mesmo com `REPO_ROOT` apontando para outro lugar de propósito (garante que não há reinterpretação indevida); caminho inexistente falha com `SystemExit` cuja mensagem inclui o caminho absoluto resolvido. Validado contra o ambiente real (`docker-compose.test.yml`, serviços já com imagem construída, sem rebuild): `run_chaos_cycle` chamado via o mesmo fluxo do `validation_window.py` (`resolve_scenario_path` seguido do subprocess com `cwd=chaos-orchestrator/`), passando `--scenario` como `chaos-orchestrator/scenarios/account_and_queue_cascade.yaml` (caminho relativo à raiz do repo, o cenário exato da issue) — sem `FileNotFoundError`, log do orquestrador confirmou `"Caos ativado em account-service"` e `"Caos ativado em onboarding-service"`, log dos dois serviços confirmou `POST /internal/chaos/config` `200 OK`, e a métrica raspada pelo Prometheus (`http_requests_total{route="/internal/chaos/config",status_code="200"}`) no `account-service` confirmou a chamada de ponta a ponta.
