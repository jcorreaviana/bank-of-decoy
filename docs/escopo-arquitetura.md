...
- **v3**: stack fechada em Python/FastAPI; geradores de fraude simples agora, evoluindo depois; agente preditivo sem RAG por enquanto, em repositório separado do `distrito-study`
...
- **v21**: lacuna identificada nas fases sugeridas — modelagem de fraude (um dos dois objetivos centrais do projeto) nunca tinha sido formalizada como fase própria. Adicionada como Fase 5 (entre agentes e fechamento de portfólio), seguindo a metodologia já validada nos projetos anteriores ("dataset define arquitetura"). Registrada também pendência de sanitização de referências cruzadas ao repositório `distrito-study` (issue futura criada), a ser executada por último, depois de todo o desenvolvimento ativo concluído.
- **v22**: durante a implementação da issue #16 (agente local), confirmado por teste real que o Claude Agent SDK (CLI `@anthropic-ai/claude-code` e o pacote Python `claude-agent-sdk`, que o envolve) autentica pela sessão já logada do plano Pro nesta máquina (`~/.claude/.credentials.json`), sem exigir `ANTHROPIC_API_KEY` paga — a restrição de API key obrigatória na documentação oficial do SDK é dirigida a produtos multi-usuário de terceiros, não a automação individual sobre o próprio projeto. Racional de custo do v13 ("sem custo variável") confirmado válido, não revertido. Invocação real usa `model="claude-sonnet-5"`, `effort="medium"`, `permission_mode="dontAsk"`, `allowed_tools=[...]` (com padrões tipo `Bash(pytest *)`), `max_turns`, todos confirmados como parâmetros reais de `ClaudeAgentOptions` via teste direto (não só documentação). `git push`/`gh pr` deliberadamente fora do `allowed_tools` do SDK — o gate de aprovação (score de risco) roda como código determinístico no wrapper Python, fora do raciocínio do LLM, nunca contornável pelo modelo.
...
Isso conecta diretamente com o que você já estudou em `distrito-study` (ReAct, guardrails, versionamento canary v1→v3) — a camada 3 é esse mesmo padrão aplicado a DevOps ao invés de RAG de catálogo.
...
- Agente preditivo: **começa sem RAG** (regras + LLM direto). RAG sobre logs/infraestrutura fica para uma fase futura, e viverá em **repositório separado** do `distrito-study` (que é RAG de catálogo, contexto diferente)
...
- Modelo: `llama3.2:3b` via Ollama, local, mesmo modelo já validado no `distrito-study`
...
- **Sanitização de referências cruzadas**: o repositório `bank-of-decoy` hoje contém menções diretas ao repositório `distrito-study` (em `docs/escopo-arquitetura.md` e em `specs/business/13-agente-preditivo-registro.md`, ex. "reaproveitar padrão ReAct do distrito-study", "mesmo modelo já validado no distrito-study"). Isso precisa ser removido/generalizado antes do projeto ser tratado como artefato de portfólio autônomo — nível de sanitização (só o nome do repo vs. também menções indiretas ao contexto) ainda a decidir quando essa limpeza for feita. Exemplo concreto já encontrado em código (docstring de um wrapper do Ollama no agente preditivo): referência direta ao caminho `distrito-study/usecases/case-one/agent.py::call_llm` — esse tipo de menção específica de caminho de arquivo de outro repo também precisa ser varrida, não só o nome do repositório em si
